...
let underkategori = [ // laver en liste over de kategorier jeg er interesseret i at analyser
    "Lammekød",
    "Koteletter, flæsk & schnitzler",
    "Nakkefilet & svinekam",
    "Hakket grise- & kalvekød",
    "Hakket grisekød",
    "Medister",
    "Andet grisekød",
    "Svinemørbrad",
    "Spareribs & barbecuekød",
    "Kylling",
]
...
data.forEach(function (allshops){ // går gennem hvor butik i datasættet, allshops repræsentere en butik
    let filteretUnderkategori = allshops.clearances.filter(function (ucat){ // filterere butikkens vare, så vi kun finder dem med kategorien mejeri og køl
        return (
            ...
            ucat.product.categories.da.startsWith("Kød & fisk") // sikre den starter med mejeri og køl
        );
    });
...
let kødogfiskspefvariant = JSON.stringify(underkategoriCount, null, 2);
fs.writeFileSync('kødogfiskvariant.json', kødogfiskspefvariant, 'utf-8');
